...
import tinker_io.network.MessageShiftTo;
...
public class GuiSmartOutput extends GuiContainer {
    ...
    public static final int BUTTON_ShiftToCast = 1;
    public static final int BUTTON_ShiftToBasin = 2;
    ...
    private GuiButton buttonShiftToCast;
    private GuiButton buttonShiftToBasin;
    ...
    public void initGui() {
        ...
        buttonShiftToCast = new GuiButton(BUTTON_ShiftToCast, guiLeft + 59,  guiTop + ySize - 114, 16, 16, ""){
            @Override
            public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks) {
                if (this.visible)
                {
        ...
        buttonShiftToBasin = new GuiButton(BUTTON_ShiftToBasin, guiLeft + 77,  guiTop + ySize - 114, 16, 16, ""){
            @Override
            public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks) {
                if (this.visible)
                {
        ...
        buttonList.add(buttonShiftToCast);
        buttonList.add(buttonShiftToBasin);
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        String name = I18n.format(BlockRegistry.smartOutput.getTranslationKey() + ".name");
        ...
        if (buttonEmptyTank.isMouseOver()) { // Tells you if the button is hovered by mouse
            String[] desc = { TextFormatting.RED + I18n.format("tio.gui.SO.toolTips.button_head")};
            List<String> temp = Arrays.asList(desc);
            drawHoveringText(temp, mouseX - guiLeft, mouseY - guiTop, fontRenderer);
        }
        if (buttonShiftToBasin.isMouseOver()) {
            String[] desc = { TextFormatting.RED + I18n.format("tio.gui.SO.toolTips.button_basin")};
            List<String> temp = Arrays.asList(desc);
            drawHoveringText(temp, mouseX - guiLeft, mouseY - guiTop, fontRenderer);
        }
        if (buttonShiftToCast.isMouseOver()) {
            String[] desc = { TextFormatting.RED + I18n.format("tio.gui.SO.toolTips.button_cast") };
            List<String> temp = Arrays.asList(desc);
            drawHoveringText(temp, mouseX - guiLeft, mouseY - guiTop, fontRenderer);
        }
    }
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        switch(button.id){
            case BUTTON_EMPTY_TANK:
                ...
                break;
            case BUTTON_ShiftToCast:
                tile.setCurrentMode(0);
                NetworkHandler.sendToServer(new MessageShiftTo(tile.getPos(), 0));
                break;
            case BUTTON_ShiftToBasin:
                tile.setCurrentMode(1);
                NetworkHandler.sendToServer(new MessageShiftTo(tile.getPos(), 1));
                break;
            default:
                break;
        }
